			...
			fs.createReadStream(file)
				...
				.on('data', async (data: JourneyRecord) => {
					...
					if (data.distance > 9 && data.duration > 9) {
						...
						if (data.departure_station_id != 997 && data.return_station_id != 997
							&& data.departure_station_id != 999 && data.return_station_id != 999
							&& data.departure_station_id != 754 && data.return_station_id != 754) {
							records.push(data);
						}
					}
				})
				.on('end', async () => {
					try {
						...
						console.log(`[Migration] ${records.length} journeys found and added.`);
						resolve(true);
					} catch (error) {
						console.error(`[Migration] Error while migrating journeys: `, error);
					}
				})
				.on('error', (error: any) => (console.error(error), reject(error)));
